...
def generate_stream(imageList, locsList, predsList, predsList2, predsTfList, tempList, predsImageList):
    ...
    for frame in camera.capture_continuous(raw_capt, format="bgr", use_video_port=True):
        ...
        cv2.rectangle(frame, (targetStartX, targetStartY),
                      (targetEndX, targetEndY), (0, 255, 255), 2)

        if not locsList.empty():
            boxes = locsList.get()
            for box in boxes:
                (startX, startY, endX, endY) = box
                cv2.rectangle(frame, (startX*4, startY*4),
                              ((startX + endX)*4, (startY + endY)*4), (0, 255, 0), 2)
            ...
            if not predsTfList.empty():
                ...
                color = (0, 255, 0) if (mask_prediction ==
                                        "Mask" and temp < 38) else (0, 0, 255)
            ...
            if last_detected is not None:
                if (datetime.now() - last_detected).total_seconds() < 3:
                    cv2.putText(frame, label, upperCorner, font,
        ...
        final_frame = cv2.resize(
            frame, (600, 1024), interpolation=cv2.INTER_AREA)
